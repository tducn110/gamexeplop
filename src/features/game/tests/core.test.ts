...
describe("straw stack core", () => {
  it("creates the initial game state from the viewport width", () => {
    ...
    expect(state.mv.w).toBe(252);
  });

  it("starts a drop only from the moving phase", () => {
    const state = createGame(390);
    const started = startDrop(state, 720);
    expect(started).toBe(true);
    expect(state.sub).toBe("dropping");
    expect(state.drop).not.toBeNull();
  });

  it("produces a perfect placement and increases score/combo", () => {
    const state = createGame(390);
    state.drop = { x: state.mv.x, y: 657, w: state.mv.w, vy: 20 };
    state.sub = "dropping";

    withMockedRandom(new Array(32).fill(0.5), () => {
      const result = updateGame(state, 16, 390, 720);
  ...
  it("ends the game when a drop misses the tower", () => {
    ...
    state.drop = { x: top.x + top.w + 20, y: 657, w: state.mv.w, vy: 20 };
    state.sub = "dropping";
...
describe("score logic", () => {
  it("returns perfect, good, and base outcomes", () => {
    expect(calculateScore(4, 0)).toMatchObject({ kind: "perfect", combo: 1 });
    expect(calculateScore(20, 1)).toMatchObject({ kind: "good", combo: 2 });
