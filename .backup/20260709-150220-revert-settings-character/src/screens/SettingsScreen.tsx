import { Dices, Settings as SettingsIcon, VolumeX, Volume2, Music, Music2 } from "lucide-react";
...
export function SettingsScreen({
  ...
  return (
    <div className="overlay-scrim" style={{ display: "grid", placeItems: "center", position: "fixed", inset: 0, zIndex: 120, background: "rgba(42,36,24,0.4)" }}>
      <div className="settingsPanelScene">
        <img className="settingsCharacter" src={character.src} alt="" aria-hidden="true" />
        <PanelFrame
          title={(
        ...
        >
          <div className="settingsPanelRows">
            <div className="settingsOptionRow settingsCharacterRow">
              <div className="settingsOptionLabel">
                <Dices size={20} />
                <span>Nhân vật</span>
              </div>
              <IconButton
                label="Random nhân vật"
                variant="solid"
                onClick={onRandomCharacter}
                className="settingsRandomButton"
              >
                <Dices size={17} />
                <span>Đổi</span>
              </IconButton>
